...
# Initialize thoughts if humanlayer is available
if command -v humanlayer >/dev/null 2>&1; then
    ...
    if humanlayer thoughts init --directory "$REPO_BASE_NAME" > /dev/null 2>&1; then
        ...
        if humanlayer thoughts sync > /dev/null 2>&1; then
            echo -e "${GREEN}✅ Thoughts searchable index created!${NC}"
        fi
    else
        echo -e "${YELLOW}⚠️  Could not initialize thoughts. Run 'humanlayer thoughts init' manually.${NC}"
    fi
fi
