...
| `linear.comment.created` (1,891)                    | webhook-handler                                                                                                                                                         | any workspace comment                                                                                      | **`daemon.mjs handleCommentWake`** — clears the escalation hold (write-gated) and re-dispatches the parked worker. One of only two real exec-core subscribers, and the best existing example of the target shape.                                                                                                               |
...
| **17** | `linear.issue.state_changed` → to_state **not in the declared partition**                          | **Freeze and go loud** — take no action, emit `linear.state.unclassified`, label `unknown-state` and raise an ask.                                                                                                                                                                                                                                                                | n/a — never guess                                                                                                                                                                                                                                                                            | yes                                                     | the event itself. Replaces today's silent no-op at `monitor.mjs:715-727`.                                                                                                                                                                               |
...
| **D15** | `failed` (any phase)                                            | any            | none — `routeStuckTicketToDelegate` → delegate, else publish an escalation + problem/call-to-action | none                                                                                      | the label-guard no-overwrite check ⚠ a guard · TARGET: lease                                                                                     |
...
| **Closure comment write fails** | **stopping is never gated on Linear** (transient infra is SYSTEM trouble, not a human ask). 3× retry, then the closure text persists in the signal and the event payload, `worker.preempt.comment-failed.<T>` fires, and the reconciler re-posts.                                                      |
