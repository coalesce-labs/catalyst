...
- **Shared files have one source and generated copies.** List a script (`scripts/<path>`, which keeps its path) a subagent prompt (`agents/<name>.md`, which lands at `assets/agents/<name>.md`), or a shared plugin reference (`references/<name>.md`, which lands at `assets/references/<name>.md`, outside the skill-shape budget that governs a skill's own `references/`) in the skill's `agents/vendor.yaml`, then run `bun scripts/packaging/cli.mjs vendor --write`; `render --write` does the same. The command copies bytes and file mode, and records what it wrote in `agents/vendor.lock.json`, so removing an entry deletes its copy on the next write. Edit the source, never a copy: `packaging-gate.yml` regenerates and fails on drift, and `scripts/packaging/__tests__/skill-self-containment.test.mjs` checks every copy byte for byte.
