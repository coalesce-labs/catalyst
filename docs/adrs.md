...
**Superseded in part by ADR-030 (2026-09-13):** the short-term layer is gone; config and long-term memory stand.
...
**Superseded by ADR-030 (2026-09-13).**
...
## ADR-030: catalyst-dev ships no hooks, and workflow context is explicit input (CTL-2306, 2026-09-13)

**Decision.** `plugins/dev` carries no hooks. The three it had — the plan-mode pair (`sync-plan-to-thoughts.sh`, `inject-plan-template.sh`), `update-workflow-context.sh`, and the `detect-bare-linear-read.sh` quota guard — are removed, together with the state they fed: `scripts/workflow-context.sh`, `register-thought.sh`, and every reader and writer of `.catalyst/.workflow-context.json`. A skill that picks up a prior document takes it explicitly: a path passed in, or the newest `thoughts/shared/` document for the ticket the run was given (the argument, or `$CATALYST_TICKET` under a phase). Under a phase, a ticket with no document yields none — never another ticket's.

**Why.** Claude Code hooks are pack-scoped, and the packaging safety gate (`scripts/packaging/core/safety-gate.mjs`) rightly vetoes every skill of a hook-bearing pack from the Codex and `.agents/skills` targets, so one hook kept all of catalyst-dev off every non-Claude harness. None of the hooks was load-bearing (a 2026-09-13 audit of every dev skill). Workflow context's whole value was carrying pointers between runs, which a disposable phase container cannot rely on and a Codex or OpenCode session never populated. The relay contract already hands each phase its ticket and prior-artifact paths. Ryan made the call on 2026-09-13, including dropping the quota guard: phase containers never run `linearis`.

**Consequences.** Supersedes ADR-004 and the short-term layer of ADR-003. Interactive runs with no argument offer the newest document on disk and ask before using it. `research-codebase` guards its Linear writes in-step (skip under `CATALYST_PHASE`, skip silently without `linearis`), the same shape `implement-plan` already had. A future cross-harness hook (telemetry first) is emitted at build time by rulesync outside `plugins/`, never added back here (CTL-2306 Phase 5); `plugins/dev/skills/__tests__/no-plugin-hooks.test.sh` fails the build if a hook layer reappears.
