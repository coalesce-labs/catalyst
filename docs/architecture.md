...
pointer to that anchor. Every affected ticket retains its escalation record, but the pointer
...
Two gaps closed at the point where the scheduler escalates a ticket:

**Gap 1 — Delegate-first routing seam.** Every escalation producer (six sites in `scheduler.mjs`
...
- **`CATALYST_DELEGATE_FIRST=shadow`**: logs a `delegate.would-route` event per eligible ticket but
  still publishes the escalation. Safe dry-run.
- **`CATALYST_DELEGATE_FIRST=enforce`**: calls `enqueueDelegateIntent`; if the queue accepts
  ...
  `result.labelled === true` so routed tickets never record a spurious escalation transition.
