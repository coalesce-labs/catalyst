...
This spec's day-one scope claimed only "well-formedness" for the `agentsSkills` bundle — nothing checked it against the actual `skills` CLI (agentskills.io) that installs it, and nothing published it anywhere a consumer could reach. CTL-2215 closed both gaps without touching this spec's model, seam, or manifest schema: `core/agentskills-spec.mjs` grades the planned emit set against `skills@1.5.23`'s real parser contract (verified by reading its bundled source, not by citation), wired into `packaging-gate.yml` as a check distinct from drift; and `.github/workflows/publish-skills.yml` pushes a graded, conformant tree to the generated sibling repo `coalesce-labs/catalyst-skills` on release. Three design questions this spec left open are now closed, verified: the CLI has no mechanism equivalent to `disable-model-invocation`, so `TARGET_CAPABILITIES.agentsSkills.canExpressInvocationConstraint` correctly stays `false` (a case-sensitive search of the whole CLI bundle for `allow_implicit_invocation` returns zero matches); `metadata.internal` is a `vercel-labs/skills` convention the CLI itself enforces (skip-on-true), which is why Catalyst's own safety gate omitting internal-exposure skills upstream — rather than emitting them flagged — is the correct, non-redundant choice; and growing the portable set (opting more `catalyst-dev` skills past `hooks.toml`'s pack-wide veto) is explicitly out of this ticket's scope, same as it was out of CTL-1461's. See `docs/architecture.md`'s Multi-Target Packaging section and `docs/releases.md`'s "Portable Skills Pack Publication" for the shipped shape; CTL-2245 still owns the shared Catalyst↔catalyst-cloud conformance fixtures, loss-report quantification, and per-harness install docs.
