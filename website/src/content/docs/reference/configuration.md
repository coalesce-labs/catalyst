...
| `catalyst.terminalSweepReap.mode` _(Layer 2)_                 | `shadow`                     | Worktree-reap intent for stalled or failed tickets that the terminal sweep confirms merged or explicitly Done. `off` disables the producer; `shadow` emits `terminalSweep.would.reap-request` and removes nothing; `enforce` emits targeted `orphans.reap-requested` for the evidence-gated reaper. `CATALYST_TERMINAL_SWEEP_REAP` overrides Layer 2, accepts `off`, `shadow`, or `enforce`, and maps `0` to `off`. Precedence is environment → Layer 2 → the conservative `shadow` default; invalid environment values resolve to `shadow`, never `enforce`. |
