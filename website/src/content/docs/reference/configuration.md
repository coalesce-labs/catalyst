...
| Sentry      | `org`, `project`, `authToken` | catalyst-dev (`sentry-research` agent)              |
...
| `CATALYST_CLOUD_BASE_URL` env / `catalyst.cloud.baseUrl` (Layer-1 or Layer-2) | Cloud API base — where every host-side write and the replica feed go. Resolved by `plugins/dev/scripts/lib/cloud-facts.mjs` (env → Layer-1 → Layer-2 → canonical). **A `*.catalyst-cloud.coalescelabs.ai` URL is REFUSED from every rung** (CTL-2300): that estate is retired, and it shipped as the built-in default in two modules until this key existed. | `https://staging.catalystcloud.dev/api/v1` |
| `CATALYST_CLOUD_ACCOUNT` env                                            | The tenant this host acts for. **No default** — `check-project-setup.sh` reports it as `Tenant identity — tenant UNRESOLVED` rather than assuming `tenant-0`, which on a customer host is the fleet owner's account.                                              | —                                                                |
| `CATALYST_HOST_DAILY_WRITE_BUDGET` env / `catalyst.cloud.hostDailyWriteBudget` | The cloud's per-host daily Linear write cap, **mirrored** for the host's own exhaustion signal — the cloud enforces its own. A non-integer, zero or negative value falls through rather than being honoured. | `3000` (the cloud's `DEFAULT_HOST_DAILY_WRITE_BUDGET`, raised from 300 by CTC-796) |
| `CATALYST_ASK_LABELS` env / `catalyst.linear.askLabels` (Layer-1 or Layer-2) | The labels every ask ticket carries on this tenant, as a JSON array (config) or a comma-separated list (env). Resolved by `plugins/dev/scripts/lib/board-vocabulary.mjs`; `ask.mjs` then maps the names to ids ON THE TARGET TEAM, because Linear labels are team-scoped and the same name has a different id per team. An empty or partly-blank declaration falls through rather than filing an ask under a blank label. | `["catalyst-ask", "ask/decision"]` |
| `CATALYST_NOT_AN_ASK_LABEL` env / `catalyst.linear.notAnAskLabel` (Layer-1 or Layer-2) | The label a human applies to release a ticket the cloud's ask-shape gate held as a false positive (CTC-1534/CTC-1752). | `catalyst-not-an-ask` |
