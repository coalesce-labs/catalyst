...
### Transient-infra exemption (CTL-1563)

A worker killed by a provider-overload burst (Anthropic 429/529) exits **before** recording a
verdict. Because the recovery-intent ledger increments `attempts` eagerly at dispatch, each such
death used to read to the exhaustion sweep as one more "dispatch without a recorded verdict" — and
at the cap the ticket was latched to a terminal `needs-human` that survives 7 days. On 2026-07-29
one burst falsely escalated ~6 healthy tickets that a delegate had to un-stick by hand.

The sweep now recognizes the transient-infra failure class and, for such a death, **refunds** the
eagerly-counted attempt, writes a `defer` intent, and emits `recovery.transient-defer` instead of
escalating. A burst that kills N in-flight workers escalates **zero** of them. A death whose reason
is structural — or that cannot be read at all — escalates exactly as before: the exemption is
scoped, and "could not look" is deliberately not "transient".

| Key | Default | Notes |
| --- | --- | --- |
| `CATALYST_TRANSIENT_INFRA_EXTRA_REASONS` | _(unset)_ | Comma-separated **extra** reason strings to treat as transient, unioned with the canonical set (`sdk-overloaded-exhausted`, `codex-rate-park-exhausted`). Exists so an operator riding out a novel provider condition can widen the exemption without a deploy. It can only **add** — there is no removal knob, because narrowing the set would reinstate the false escalation. Matching is exact (trimmed) string membership: there is no globbing, so `*` matches nothing. Blank entries are ignored. |

Backoff is keyed on its own `transient-infra` fix class in `.recovery-fix-failures/`, so a provider
outage never consumes the retry budget metered against a genuine `bounded-llm`/seam fix failure.
The refund — not the exemption — is what the exponential window throttles: while the window is
open the ticket still defers and still never escalates, it just stops buying fresh retries. The
window reuses `CATALYST_RECOVERY_FIX_BACKOFF_*` (threshold, base, max).
...
| `CATALYST_BH_OVERLOAD_ATTRIBUTION` _(env var)_  | `shadow`                  | CTL-1563 (AC4). Provider-overload attribution mode: `off` omits the invariant entirely (the `off` invariant set stays byte-identical to before this shipped), `shadow` reports a detected burst but keeps the invariant unobservable and proposes nothing, and `enforce` fails the invariant on a fresh burst and proposes ONE **escalate-only** tier-3 `escalate-provider-overload` move. It is read-only by design — it never dispatches a recovery delegate, because the correct response to an outage is to let it clear. An unrecognized value falls back to `shadow`, never to `off`. |
| `CATALYST_BH_OVERLOAD_BURST_MIN`                | `3`                       | CTL-1563. Minimum number of **exhausted** overload deaths (`execution-core.sdk.overloaded` with `exhausted:true`) inside the window before the board's stalled work is attributed to a provider outage rather than to the tickets. A single flaky overload is not a burst. |
| `CATALYST_BH_OVERLOAD_WINDOW_MS`                | `7200000` (2 h)           | CTL-1563. Recency window for that evidence. A burst whose newest death is older than this — or that carries no usable timestamp — is **not** attributed to the current board: an outage that ended hours ago explains nothing about the wedge right now. |
