...
1. **Operator credential step:** obtain the cloud credential from the operator who manages the
   ...
   `TOKEN_VAR` to that value by hand. After step 2, `catalyst doctor`'s `replica-token` line reports
   the name this host actually resolved — use it to confirm you provisioned the right one.
...
`CATALYST_BH_REPLICA` independently controls the replica completeness invariant (`off`, `shadow`, or
