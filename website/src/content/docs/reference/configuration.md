...
| `broker.cacheReconcile.mode`                                 | `off`                        | Periodically reconcile the broker's `ticket_state` cache from the Linear replica. Resolution is env `CATALYST_CACHE_RECONCILE` → Layer 2 → Layer 1 → default. Use `shadow` to emit `cache.reconcile.*` would-write events without mutation; after observing those events for a week, operators may promote to `enforce`. Invalid values fail closed to `off`. |
| `broker.cacheReconcile.intervalSeconds`                      | `600`                        | Reconcile cadence in seconds. Env `CATALYST_CACHE_RECONCILE_INTERVAL_MS` overrides in milliseconds. |
| `broker.cacheReconcile.perPassCap`                           | `250`                        | Maximum tickets examined per pass. Env `CATALYST_CACHE_RECONCILE_CAP` overrides. |
