...
or command, and the widened row still passes through every pre-existing gate. The same widening
lands in the hourly `orphan-sweep.sh` vector 1 as an **additional** branch alongside the legacy
...
the script's own defaults are what production sees, because the installed plist sets none):
...
| `SWEEP_PROC_WIDEN_MAX_KILLS`    | `5`      | Per-run cap on widened kills (`0` = uncapped), mirroring vector 2's `SWEEP_MAX_REMOVALS`. Counted on the enforcing path only, so `shadow` still reports the full candidate set. Overflow is logged and deferred to the next run. |
...
The shell branch additionally refuses to run at all when `SWEEP_WT_ROOT` itself is missing: a
renamed or unmounted root makes *every* cwd beneath it look deleted in the same pass, which is a
root-level fault rather than N independent orphans.
