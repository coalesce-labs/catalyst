...
| `batchSize`          | `100`                  | Max events per HTTP request. A flush drains the whole buffer and sends it in ≤ `batchSize` chunks, so no single request exceeds the destination's payload limit (CTL-1506). |
