...
| `CATALYST_TRIAGE_REQUEST_ESCALATE`                            | `shadow`                     | Bounded triage-request escalation mode: `off`, `shadow`, or `enforce`. Enforce routes through the delegate-first seam once per request episode. |
| `CATALYST_TRIAGE_REQUEST_ESCALATE_MS`                         | `2700000`                    | Age in milliseconds (45 minutes by default) before an unfulfilled triage request becomes escalation-eligible. |
| `CATALYST_TRIAGE_REQUEST_TTL_MS`                              | `86400000`                   | Stale triage-request retention in milliseconds (24 hours by default). |
