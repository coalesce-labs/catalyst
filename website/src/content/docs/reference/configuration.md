...
| `MONITOR_STRICT_LOOPBACK=1` | unset | Drops the family-ambiguous `localhost` **name** when the server binds a single address family (the unambiguous literals stay). `localhost` resolves per the browser's choice, so under an IPv4-only bind a service squatting `[::1]:<port>` could serve a page whose `Origin` is `http://localhost:<port>` and POST to the IPv4 monitor. It is trusted by default because dropping it would 403 the commonest local path (`http://localhost:7400`) on every IPv4-only deployment — a certain inertness failure — to close an attack that first needs local code execution. **Binding dual-stack (`::`) removes the squat window entirely and is the better fix.** Accepts `1`/`true`/`yes`/`on`. Safe with `bun run dev:ui` — the Vite proxy rewrites to the unambiguous `http://127.0.0.1:7400`, which strict mode keeps. |
