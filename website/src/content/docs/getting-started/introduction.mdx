---
title: Introduction
description: Catalyst chains commands, agents, skills, and hooks into repeatable development workflows for Claude Code.
sidebar:
  order: 0
---
...
<CardGrid>
  <Card title="Commands" icon="rocket">
    Structured workflows you invoke with a slash command. Each command orchestrates a multi-step process — spawning agents, reading context, interacting with you, and saving artifacts. Commands are how work gets done.
  </Card>
  <Card title="Agents" icon="magnifier">
    Specialized roles that commands delegate to. A locator agent finds files. An analyzer agent reads implementation details. A pattern-finder agent discovers reusable code. Commands spawn these in parallel to gather information fast.
  </Card>
  <Card title="Skills" icon="open-book">
    Reusable knowledge that teaches Claude how to approach specific tasks — debugging strategies, commit conventions, writing styles. Skills shape behavior without you having to re-explain preferences every session.
  </Card>
  <Card title="Hooks" icon="setting">
    Automation that runs at specific moments in a workflow. A hook can enforce linting before a commit, run tests after implementation, or validate that a plan meets criteria before execution starts. Guardrails, not guardrails you have to remember.
  </Card>
</CardGrid>

These four pieces compose. A command spawns agents, agents use skills, hooks enforce quality at each gate. The result is a workflow — not a conversation, but a structured process that produces predictable output.
...
<CardGrid>
  ...
  <Card title="Your First Workflow" icon="right-arrow">
    Walk through the full research-plan-implement cycle.

    [Try it →](/getting-started/first-workflow/)
  </Card>
</CardGrid>
