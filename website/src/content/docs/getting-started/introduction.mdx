...
<CardGrid>
  ...
  <Card title="Memory" icon="document">
    Persistent, git-backed storage that survives across sessions. Catalyst uses the [HumanLayer thoughts system](https://humanlayer.dev) as long-term memory — research findings, implementation plans, handoff checkpoints, and PR descriptions all persist in `thoughts/shared/`. Any agent, in any session, can come back and understand what happened before. This is what makes multi-day features and team handoffs possible.
  </Card>
...
- **Phase-agents dispatch (opt-in)** — For orchestration runs, Catalyst can split each ticket across a sequence of short-lived background jobs defined by the default [workflow descriptor](/reference/orchestration/workflows/) (triage → research → plan → implement → verify → review → pr → monitor-merge → monitor-deploy). Each job runs as `claude --bg`, draws from the Max 20x subscription pool, and can be assigned a different model and reasoning effort. See [Phase agents](/reference/orchestration/phase-agents/) for the full pipeline.
