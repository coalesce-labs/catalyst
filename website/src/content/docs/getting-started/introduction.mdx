...
- **Phase-agents dispatch (opt-in)** — For orchestration runs, Catalyst can split each ticket across nine short-lived background jobs (triage → research → plan → implement → verify → review → pr → monitor-merge → monitor-deploy). Each job runs as `claude --bg`, draws from the Max 20x subscription pool, and can be assigned a different model. See [Phase agents](/reference/orchestration/phase-agents/) for the full pipeline.
