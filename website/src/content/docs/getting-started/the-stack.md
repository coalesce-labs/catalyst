...
- **Phase-agents dispatch** — Catalyst's native answer to long-running worker sessions. The orchestrator can dispatch each ticket as nine short-lived `claude --bg` jobs (one per phase) instead of one long `claude -p` oneshot. Each phase reads the previous phase's artifact, does one thing, and exits — so context stays small and per-phase model assignment becomes possible. See [Phase agents](/reference/orchestration/phase-agents/) for the pipeline and cost economics.
