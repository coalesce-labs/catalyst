...
| `phase.yield.reap-requested` / `phase.predecessor.reap-requested` / `phase.supersede.reap-requested` / `phase.revive.reap-requested` / `phase.abort.reap-requested` / `phase.reclaim.reap-requested` / `phase.reconcile.reap-requested` / `phase.terminal.reap-requested` | execution-core reap-intent producers + reaper, CTL-649 | Reap-intent requests — a worker bowed out (yield), a successor wants its predecessor reaped, a stale signal was superseded, revive's defensive-kill, the abort-worker path, the reclaim happy-path stop (CTL-661), the per-ticket reconciliation sweep's non-canonical-session stop (CTL-661), or a terminal worker with no successor dispatch — including the CTL-736 `no-progress-stopped` STOP, where a dead worker that made zero forward progress is stopped (never respawned) via this single-target busy-OK reap (CTL-695) |
