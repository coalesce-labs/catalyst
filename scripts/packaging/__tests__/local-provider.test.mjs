...
describe("renderPluginPack — round-trip against every real plugin", () => {
  ...
  test("discovers exactly 4 plugin directories", () => {
    expect(pluginRelPaths.length).toBe(4);
  });
  ...
  test("sum of per-plugin skill counts across all 4 plugins is a positive, live-computed total (not hardcoded)", () => {
    let total = 0;
