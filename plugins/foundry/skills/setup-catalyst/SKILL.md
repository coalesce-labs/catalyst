...
| Single-harness repo (missing `@AGENTS.md` bridge / non-portable skills) — check-project-setup.sh §10 (CTL-1530) | Run `bash ${CATALYST_DEV_SCRIPTS}/migrate-dual-harness.sh --repo . --fix`. Idempotent and safe to run without asking — it only wires the `.agents/skills` ↔ `.claude/skills` symlink, creates a thin `@AGENTS.md` bridge for a Codex-only repo, and appends the `AGENTS.md` skills pointer. If it exits `11` (a monolithic `CLAUDE.md` that needs the intelligent portable/Claude-specific split), that step requires model judgment — hand off to the `catalyst-foundry:migrate-dual-harness` skill rather than attempting the split inline. |
