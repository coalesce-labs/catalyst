...
| Single-harness repo (missing `@AGENTS.md` bridge / non-portable skills) — run this **proactively every setup**, not only when check-project-setup.sh §10 flags it (§10 is a hot-path, TTL-gated warning, and a direct `catalyst-foundry:setup-catalyst` invocation for an upgrade never runs check-project-setup.sh at all, so relying on the §10 flag alone would silently skip the migration) | From the project root dry-run the classifier: `bash "${CATALYST_DEV_SCRIPTS}/migrate-dual-harness.sh" --repo . --quiet`. rc `0` → already dual-harness, nothing to do. rc `10` → apply `bash "${CATALYST_DEV_SCRIPTS}/migrate-dual-harness.sh" --repo . --fix` (idempotent and safe to run without asking — it only wires the `.agents/skills` ↔ `.claude/skills` symlink, creates a thin `@AGENTS.md` bridge for a Codex-only repo, and appends the `AGENTS.md` skills pointer). rc `11` (a monolithic `CLAUDE.md` that needs the intelligent portable/Claude-specific split) — **setup-catalyst cannot perform this split itself** (the `catalyst-foundry:migrate-dual-harness` skill is `disable-model-invocation: true` and cannot be auto-invoked as a handoff). **Stop here** and tell the user to run `/catalyst-foundry:migrate-dual-harness` themselves to complete the split. rc `4` (ambiguous skills state) → the classifier's explanation of the conflict arrives on **stderr** and is NOT suppressed by `--quiet` (which only silences the informational stdout output) — surface that stderr diagnostic to the user **verbatim**; never force a resolution by hand. |
