...
| Agent house-rules block missing or stale in `AGENTS.md` / `CLAUDE.md` (the "Working the Loop" reflexes — check-project-setup.sh §9) | Run `bash plugins/dev/scripts/ensure-agent-house-rules.sh --fix`. Idempotent: seeds the canonical block into the doc the driving agent loads (AGENTS.md when CLAUDE.md is an `@AGENTS.md` bridge, else CLAUDE.md), creating an `AGENTS.md` + thin bridge if the repo has neither, and re-syncs a stale block to the current template. Safe, local, reversible — this is the auto-seed that makes every newly-enrolled repo carry the reflexes, so run it without asking. |
