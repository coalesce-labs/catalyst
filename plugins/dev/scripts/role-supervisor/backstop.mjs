...
//
//   1. the holding-reply SENTINEL raises the tagged "steward/<slug> is being
//      restarted" alert at the 15-minute silence mark and asks the supervisor
//      to restart the role;
//   2. the DEAD-MAN alarm raises ONE alert (catalyst.alert.raised) when
//      there is no concierge heartbeat AND no turn for 30 minutes.
//
...
// is concierge-specific (heartbeat 30m AND turn 30m). Kept as their own
 ...
 * heartbeat AND the last turn are >= 30 minutes old, and the alarm has
 * not already pushed this episode.
 *
 * A `null` age (missing heartbeat / no turn ever) counts as DEAD, never
 ...
 * what keeps a merely-restarting concierge (fresh turn) or a quiet-but-
 * alive one (fresh heartbeat) from paging the human.
 *
 * @param {{conciergeHbAgeMs: number|null, lastTurnAgeMs: number|null, alreadyPushed: boolean}} arg
 * @returns {boolean}
 */
export function deadManShouldFire({ conciergeHbAgeMs, lastTurnAgeMs, alreadyPushed = false } = {}) {
  ...
  const turnDead = !(typeof lastTurnAgeMs === "number") || lastTurnAgeMs >= DEAD_MAN_AFTER_MS;
