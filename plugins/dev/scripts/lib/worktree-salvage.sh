...
salvage_worktree() {
  ...
  local dir ts bundle patch untar
  ...
  bundle="${dir}/${ticket}-${ts}.bundle"
  patch="${dir}/${ticket}-${ts}.patch"
  untar="${dir}/${ticket}-${ts}-untracked.tar"

  local commits_saved=0 files_changed=0 untracked_count=0 err=""
  local saved_bundle="" saved_patch="" saved_untar=""

  # (a) Unpushed commits → bundle. `HEAD --not --remotes` = reachable from HEAD,
  #     on no remote. `git bundle create` fails ("empty bundle") when nothing
  #     qualifies — that's the clean case, not an error.
  local tmp_b="${bundle}.tmp.$$"
  if git -C "$wt" bundle create "$tmp_b" HEAD --not --remotes >/dev/null 2>&1; then
    if mv -f "$tmp_b" "$bundle" 2>/dev/null; then
      saved_bundle="$bundle"
      commits_saved="$(git -C "$wt" rev-list --count HEAD --not --remotes 2>/dev/null || echo 0)"
    else
      rm -f "$tmp_b" 2>/dev/null || true
      err="bundle-mv-failed"
    fi
  else
    rm -f "$tmp_b" 2>/dev/null || true   # empty-bundle: expected, not an error
  fi

  # (b) Tracked uncommitted diff (staged + unstaged) → patch.
  if ! git -C "$wt" diff --quiet HEAD 2>/dev/null; then
    local tmp_p="${patch}.tmp.$$"
    if git -C "$wt" diff HEAD >"$tmp_p" 2>/dev/null && [[ -s "$tmp_p" ]] && mv -f "$tmp_p" "$patch" 2>/dev/null; then
      saved_patch="$patch"
