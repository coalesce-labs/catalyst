...
describe("readClusterLivenessFromLoki (CTL-1420 #17) — fail-open", () => {
  ...
  test("two-query: liveness from query A + tickets enriched from query B (per host)", async () => {
    // Query A (no tickets filter) returns host+ts for BOTH hosts; Query B (tickets filter)
    // returns only the host WITH a non-empty set. The reader merges B's tickets onto A.
    ...
    const out = await readClusterLivenessFromLoki({ lokiUrl: "http://loki:3100", fetcher, nowMs: 1783451100000 });
    expect(Object.keys(out).sort()).toEqual(["mini", "mini-2"]); // liveness for BOTH (from A)
    expect(out.mini.in_flight_tickets).toEqual(["CTL-1", "CTL-2"]); // tickets from B
    expect(out["mini-2"].in_flight_tickets).toEqual([]); // no tickets → []
  });
  ...
  test("tickets-enrichment (query B) failure does NOT break liveness (query A stands)", async () => {
    ...
    expect(out.mini).toBeDefined(); // liveness survived the B failure
