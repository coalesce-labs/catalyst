...
export function applyLabel({ ticket, label, exec = defaultExec, readLabels = null }) {
  try {
    ...
    if (writeRes.code !== 0) {
      ...
      const details = { ticket, label, code: writeRes.code, reason, stderr: writeRes.stderr };
      if (reason === "circuit-open") {
        log.debug(details, "linear-write: label write failed (exit non-zero)");
      } else {
        log.warn(details, "linear-write: label write failed (exit non-zero)");
      }
