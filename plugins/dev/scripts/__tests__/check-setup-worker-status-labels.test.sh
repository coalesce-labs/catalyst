...
make_project "$P1"; make_secrets "$X1"; install_ws_curl "$B1" "$L1" "full"
: > "$L1"
NOW_TS="$(date +%s)"
...
make_project "$P2"; make_secrets "$X2"; install_ws_curl "$B2" "$L2" "full"
: > "$L2"; mkdir -p "${X2}/catalyst"
...
make_project "$P3"; make_secrets "$X3"; install_ws_curl "$B3" "$L3" "missing-needs-input"
: > "$L3"; mkdir -p "${X3}/catalyst"
...
make_project "$P4"; install_ws_curl "$B4" "$L4" "full"
: > "$L4"
mkdir -p "${X4}/catalyst"
...
make_project "$P5"; make_secrets "$X5"; install_ws_curl "$B5" "$L5" "no-group"
: > "$L5"; mkdir -p "${X5}/catalyst"
