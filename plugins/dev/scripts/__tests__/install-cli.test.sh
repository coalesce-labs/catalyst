...
# CTC-2981: every run below touches LaunchAgents only inside SCRATCH, never the
# operator's ~/Library/LaunchAgents. Test 24e unsets this to exercise $HOME.
export CATALYST_LAUNCH_AGENTS_DIR="$SCRATCH/launch-agents-isolated"
...
setup_source() {
  ...
  rm -rf "$src"
  mkdir -p "$src"
  for f in catalyst-events catalyst-execution-core catalyst-otel-forward \
...
for cli in catalyst-events catalyst-execution-core catalyst-otel-forward \
...
run "re-install with new source updates targets" bash -c "
  ...
  target=\$(readlink '$BIN1/catalyst-events')
  [[ \"\$target\" = '$SRC2/catalyst-events' ]]
"
...
run "uninstall removes catalyst-* links" bash -c "
  ...
  [[ ! -e '$BIN5/catalyst-events' && ! -e '$BIN5/catalyst-session' ]]
"
...
run "auto-detect source from script's own dir" bash -c "
  ...
  target=\$(readlink '$BIN7/catalyst-events')
  [[ \"\$target\" = '$AUTO_SRC/catalyst-events' ]]
"
...
cat > "$HOME20/.zshrc" <<'RC'
# misc setup
alias catalyst-events=/Users/old/code/catalyst/plugins/dev/scripts/catalyst-events
RC
...
# ── 24. CTL-339: preserves a non-catalyst-* source symlink the user placed ──
# (Before CTL-2306 this case used register-thought; that name is now a RETIRED
# launcher the installer removes on purpose — see case 24b — so the preservation
# property is pinned on a name the installer has never owned.)
...
echo '#!/bin/sh' > "$FAKE_CLONE24/my-helper.sh"
chmod +x "$FAKE_CLONE24/my-helper.sh"
ln -s "$FAKE_CLONE24/my-helper.sh" "$BIN24/my-helper"
setup_source "$SCRATCH/plugin24/scripts"
run "preserves a non-catalyst-* source symlink (sweep is limited to catalyst-*)" bash -c "
  ...
  target=\$(readlink '$BIN24/my-helper' 2>/dev/null)
  [[ \"\$target\" = '$FAKE_CLONE24/my-helper.sh' ]]
"

# ── 24b. CTL-2306: retired launchers are cleaned up on install AND uninstall ─
# register-thought and workflow-context left CLI_ENTRIES, so neither reinstall
# nor --uninstall visits them any more. An upgrade must still remove the ones
# the installer wrote (a symlink into plugins/dev/scripts/<name>.sh, or a
# wrapper shim carrying the marker) and must leave a same-named file the user
# owns alone (Codex review on #4132).
retired_fixture() {
  local home="$1" bin="$2" clone="$3"
  mkdir -p "$bin" "$clone" "$home/.local/bin"
  echo '#!/bin/sh' > "$clone/register-thought.sh"
  ln -s "$clone/register-thought.sh" "$bin/register-thought"
  printf '#!/usr/bin/env bash\n# Auto-generated by catalyst install-cli.sh (version-auto, checkout-preferring) — do not edit\n' > "$bin/workflow-context"
  printf '#!/usr/bin/env bash\n# my own script, same name\necho mine\n' > "$home/.local/bin/workflow-context"
  chmod +x "$bin/workflow-context" "$home/.local/bin/workflow-context"
}
HOME24B="$SCRATCH/home24b"; BIN24B="$HOME24B/.catalyst/bin"; CLONE24B="$SCRATCH/clone24b/plugins/dev/scripts"
retired_fixture "$HOME24B" "$BIN24B" "$CLONE24B"
setup_source "$SCRATCH/plugin24b/scripts"
run "install removes retired installer-owned launchers (symlink + marked shim)" bash -c "
  HOME='$HOME24B' CATALYST_CLI_SOURCE='$SCRATCH/plugin24b/scripts' CATALYST_BIN_DIR='$BIN24B' \
    $INSTALL_CLI > '$SCRATCH/out24b' 2>&1
  [[ ! -e '$BIN24B/register-thought' && ! -L '$BIN24B/register-thought' && ! -e '$BIN24B/workflow-context' ]]
"
run "install leaves a user-owned file that shares a retired name" bash -c "
  grep -q 'echo mine' '$HOME24B/.local/bin/workflow-context'
"
HOME24C="$SCRATCH/home24c"; BIN24C="$HOME24C/.catalyst/bin"; CLONE24C="$SCRATCH/clone24c/plugins/dev/scripts"
retired_fixture "$HOME24C" "$BIN24C" "$CLONE24C"
run "uninstall removes retired installer-owned launchers too" bash -c "
  HOME='$HOME24C' CATALYST_BIN_DIR='$BIN24C' $INSTALL_CLI --uninstall > '$SCRATCH/out24c' 2>&1
  [[ ! -L '$BIN24C/register-thought' && ! -e '$BIN24C/workflow-context' ]] \
    && grep -q 'echo mine' '$HOME24C/.local/bin/workflow-context'
"

# ── 24d. CTC-2981: catalyst-comms is retired too ───────────────────────────
# It left CLI_ENTRIES with its channel system. Its source name has no .sh
# suffix, so the retire match must accept plugins/dev/scripts/<name> as well.
HOME24D="$SCRATCH/home24d"; BIN24D="$HOME24D/.catalyst/bin"; CLONE24D="$SCRATCH/clone24d/plugins/dev/scripts"
mkdir -p "$BIN24D" "$CLONE24D" "$HOME24D/.local/bin"
echo '#!/bin/sh' > "$CLONE24D/catalyst-comms"
ln -s "$CLONE24D/catalyst-comms" "$HOME24D/.local/bin/catalyst-comms"
printf '#!/usr/bin/env bash\n# Auto-generated by catalyst install-cli.sh (version-auto, checkout-preferring) — do not edit\n' > "$BIN24D/catalyst-comms"
chmod +x "$BIN24D/catalyst-comms"
setup_source "$SCRATCH/plugin24d/scripts"
run "install removes a retired catalyst-comms shim and its ~/.local/bin symlink" bash -c "
  HOME='$HOME24D' CATALYST_CLI_SOURCE='$SCRATCH/plugin24d/scripts' CATALYST_BIN_DIR='$BIN24D' \
    $INSTALL_CLI > '$SCRATCH/out24d' 2>&1
  [[ ! -e '$BIN24D/catalyst-comms' && ! -L '$HOME24D/.local/bin/catalyst-comms' ]]
"

# ── 24e. CTC-2981: the channel-watcher LaunchAgent is retired ──────────────
# A fake HOME holds the old plist; `launchctl` and `uname` are shims on PATH, so
# the real launchctl is never called. The shim records its argv.
HOME24E="$SCRATCH/home24e"; SHIM24E="$SCRATCH/shim24e"
mkdir -p "$HOME24E/Library/LaunchAgents" "$SHIM24E"
touch "$HOME24E/Library/LaunchAgents/ai.coalesce.catalyst-channel-watcher.plist"
touch "$HOME24E/Library/LaunchAgents/com.example.keep-me.plist"
cat > "$SHIM24E/launchctl" <<SHIM
#!/usr/bin/env bash
echo "\$*" >> "$SCRATCH/launchctl24e.log"
exit 3 # not loaded — the installer must tolerate it
SHIM
printf '#!/usr/bin/env bash\necho Darwin\n' > "$SHIM24E/uname"
chmod +x "$SHIM24E/launchctl" "$SHIM24E/uname"
setup_source "$SCRATCH/plugin24e/scripts"
run "install boots out the channel-watcher LaunchAgent and removes its plist" bash -c "
  env -u CATALYST_LAUNCH_AGENTS_DIR PATH='$SHIM24E:$PATH' HOME='$HOME24E' \
    CATALYST_CLI_SOURCE='$SCRATCH/plugin24e/scripts' CATALYST_BIN_DIR='$HOME24E/.catalyst/bin' \
    $INSTALL_CLI > '$SCRATCH/out24e' 2>&1
  grep -qE '^bootout gui/[0-9]+/ai\.coalesce\.catalyst-channel-watcher\$' '$SCRATCH/launchctl24e.log' \
    && [[ ! -e '$HOME24E/Library/LaunchAgents/ai.coalesce.catalyst-channel-watcher.plist' ]] \
    && [[ -e '$HOME24E/Library/LaunchAgents/com.example.keep-me.plist' ]]
"
run "a second install is a no-op: no plist, no launchctl call" bash -c "
  : > '$SCRATCH/launchctl24e.log'
  env -u CATALYST_LAUNCH_AGENTS_DIR PATH='$SHIM24E:$PATH' HOME='$HOME24E' \
    CATALYST_CLI_SOURCE='$SCRATCH/plugin24e/scripts' CATALYST_BIN_DIR='$HOME24E/.catalyst/bin' \
    $INSTALL_CLI > '$SCRATCH/out24e2' 2>&1
...
echo '#!/bin/sh' > "$FAKE_CLONE26/catalyst-execution-core"
chmod +x "$FAKE_CLONE26/catalyst-execution-core"
ln -s "$FAKE_CLONE26/catalyst-execution-core" "$LOCAL26/catalyst-execution-core"
...
run "sweeps symlink shim from ~/.local/bin" bash -c "
  [[ ! -e '$LOCAL26/catalyst-execution-core' ]]
"
