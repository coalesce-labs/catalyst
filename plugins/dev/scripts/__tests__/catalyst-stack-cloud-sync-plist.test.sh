#!/usr/bin/env bash
# catalyst-stack-cloud-sync-plist.test.sh — CTL-1394. Unit-test the PURE
# render_cloud_sync_plist function (no filesystem / launchctl side effects).
# Run: bash plugins/dev/scripts/__tests__/catalyst-stack-cloud-sync-plist.test.sh
set -uo pipefail
...
# ── host_name pinned ──────────────────────────────────────────────────────────
PLIST="$(render_cloud_sync_plist "/path/to/launch.sh" "mini-2")"

check "Label is the cloud-sync agent" has "<string>ai.coalesce.catalyst-cloud-sync</string>"
...
check "stdout → cloud-sync.log" has "cloud-sync.log"
...
PLIST="$(render_cloud_sync_plist "/x/launch.sh" "")"
