...
assert_doc_has_re() {
	...
	if grep -qiE -- "$pattern" "$REPO_ROOT/$file"; then
		...
		FAILURES=$((FAILURES + 1))
		echo "  FAIL: $label (regex not found in $file): $pattern"
	fi
}
...
if [ -f "$REPO_ROOT/$CONFIG_TEMPLATE" ]; then
	if jq -e . "$REPO_ROOT/$CONFIG_TEMPLATE" >/dev/null 2>&1; then
		PASSES=$((PASSES + 1))
		echo "  PASS: config.template.json is valid JSON"
	else
		FAILURES=$((FAILURES + 1))
		echo "  FAIL: config.template.json is not valid JSON"
	fi
else
	PASSES=$((PASSES + 1))
	echo "  PASS: config.template.json not present (skipped)"
fi
