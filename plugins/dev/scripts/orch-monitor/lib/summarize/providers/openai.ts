...
export const openaiProvider: SummarizeProvider = {
  name: "openai",
  async summarize(args: SummarizeArgs): Promise<SummarizeResult> {
    ...
    const resp = await fetcher(OPENAI_URL, {
      method: "POST",
      headers: {
    ...
    if (!resp.ok) {
      throw new Error(`openai provider returned ${String(resp.status)}`);
    }
    ...
    try {
      ...
      throw new Error("openai provider returned non-JSON body");
    }
