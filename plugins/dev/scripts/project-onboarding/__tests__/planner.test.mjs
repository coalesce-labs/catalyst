...
const stepRefs = (steps) => steps.map(({ id, scope, targetId }) => ({ id, scope, ...(targetId ? { targetId } : {}) }));
...
test("excludes a developer controller from execution targets until policy and node class both permit it", () => {
  ...
  assert.deepEqual([...new Set(excluded.filter((step) => step.scope === "target").map((step) => step.targetId))], ["mini-1"]);
  assert.deepEqual([...new Set(permitted.filter((step) => step.scope === "target").map((step) => step.targetId))], ["laptop", "mini-1"]);
});
...
test("keeps offline cluster targets pending and reports incomplete required-step convergence", () => {
  ...
  assert.deepEqual(offlineSteps.map(({ status, reason, repairable }) => ({ status, reason, repairable })), [
  ...
  assert.deepEqual(readiness, {
    ...
    targets: [
      { targetId: "mini-1", ready: false, requiredSteps: { total: 5, satisfied: 0, pending: 5, blocked: 0 } },
      { targetId: "mini-2", ready: false, requiredSteps: { total: 5, satisfied: 0, pending: 5, blocked: 0 } },
    ],
  });
});
...
test("treats blocked and pending required steps as not ready while not_applicable is neutral", () => {
  ...
  assert.deepEqual(readiness, {
    ...
    targets: [
      { targetId: "laptop", ready: true, requiredSteps: { total: 0, satisfied: 0, pending: 0, blocked: 0 } },
...
test("returns byte-stable JSON and changes rerun statuses only from observations", () => {
  ...
  assert.equal(repaired.find((step) => step.id === "target.execution_registry")?.status, "satisfied");
});
